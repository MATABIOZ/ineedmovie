...
export const VoteAverage: FC<IVoteAverageProps> = ({ voteAverage }) => {
  const { getTheme, themeType } = useContext(ColorThemeContext);
  const colors = getTheme(themeType);

  const roundedVoteAverage = (Math.round(voteAverage * 10) / 10).toFixed(1);
  const [voteAverageBorderColor, setVoteAverageBorderColor] =
    useState<string>("");
  ...
  return (
    <StyledVoteAverage
      $colors={colors}
      $ratingBorderColor={voteAverageBorderColor}
    >
      <StarIcon />
      {roundedVoteAverage}
    </StyledVoteAverage>
