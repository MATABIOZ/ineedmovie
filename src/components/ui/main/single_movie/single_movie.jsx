...
import { StyledMovieCastSwiperContainer, StyledMovieInfo, StyledMovieInfoWrapper, StyledMovieItem, StyledMovieItemsTitle, StyledMovieItemsWrapper, StyledMovieText, StyledMovieTitle, } from "./single_movie.styled";
export const SingleMovie = () => {
    ...
    return (<>
      ...
      {movie.id === Number(params.movie_id) && (<>
          <Banner movieId={movie.id} title={movie.title} backgroundLink={`https://image.tmdb.org/t/p/original${movie.backdrop_path}`} voteAverage={movie.vote_average} releaseDate={movie.release_date} inSwiper={false}/>
          ...
          <ContentWrapper groupType={"info"}>
            <StyledMovieInfo>
              <StyledMovieInfoWrapper $colors={colors}>
                ...
                <StyledMovieItemsWrapper>
                  {movie.genres.map((item) => (<StyledMovieItem key={item.id} $colors={colors}>
                      {item.name}
                    </StyledMovieItem>))}
                </StyledMovieItemsWrapper>
              ...
              <StyledMovieInfoWrapper $colors={colors}>
                ...
                <StyledMovieItemsWrapper>
                  {movie.production_countries.map((item) => (<StyledMovieItem key={item.iso_3166_1} $colors={colors}>
                      {item.name}
                    </StyledMovieItem>))}
                </StyledMovieItemsWrapper>
              </StyledMovieInfoWrapper>
              {movie.homepage && (<StyledMovieInfoWrapper $colors={colors}>
                  <StyledMovieItemsTitle $colors={colors}>
                    Home Page
                  </StyledMovieItemsTitle>
                  <StyledMovieItemsWrapper>
            ...
            <StyledMovieCastSwiperContainer>
              <CustomSwiper swiperFor={"cardsForCast"} swiperClassPostfix={"cast"} castArr={movie.credits.cast}/>
            </StyledMovieCastSwiperContainer>
          </ContentWrapper>
        </>)}
      <Footer />
